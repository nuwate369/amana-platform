...
/** محادثة السائقة مع راكبتها أثناء الرحلة (Realtime). */
    ...
    >
      <View className="h-16 flex-row items-center justify-between border-b border-neutral-200 px-4 dark:border-neutral-800">
        ...
        <Text className="font-plex-bold text-lg text-neutral-900 dark:text-neutral-50">
          {name ? `محادثة ${name}` : 'محادثة الراكبة'}
        </Text>
          ...
          >
            {messages.length === 0 ? (
              <View className="mt-24 items-center gap-2">
                <MaterialIcons name="chat-bubble-outline" size={44} color={driverNavy[300]} />
                <Text className="font-plex text-sm text-neutral-400">ابدئي المحادثة مع راكبتك</Text>
              </View>
            ) : (
