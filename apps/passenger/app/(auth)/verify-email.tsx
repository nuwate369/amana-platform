 ...
 * البريد يأتي كوسيط من التسجيل أو من الدخول (لحساب غير مفعّل). عند إدخال 6 أرقام
 * يُستدعى verifyOtp تلقائيًّا؛ نجاحه يُنشئ الجلسة فتنتقل الراكبة للرئيسية.
 * التخطيط متمركز داخل KeyboardAvoidingView كي لا يختفي الحقل خلف لوحة المفاتيح.
 */
export default function VerifyEmailScreen() {
  ...
  async function verify(token: string) {
    ...
    // نجاح ⇐ أُنشئت الجلسة؛ نعود للجذر وبوّابة التوجيه تقرّر الوجهة (رئيسية/انتظار).
    ...
    <SafeAreaView className="flex-1 bg-neutral-50 dark:bg-neutral-900">
      ...
      >
        <View className="h-24 w-24 items-center justify-center rounded-full border-2 border-brand-100 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
          <MaterialIcons name="mark-email-unread" size={48} color={passengerPurple[700]} />
        </View>
        ...
        <Pressable
          ...
          className={`h-14 w-full flex-row items-center justify-center gap-2 rounded-xl active:scale-[0.98] ${
            canVerify ? 'bg-brand-600' : 'bg-neutral-300 dark:bg-neutral-700'
          }`}
        >
          {verifying ? (
        ...
        {/* إعادة إرسال الرمز — معطّل مع عدّاد تنازلي حتى انقضاء دقيقتين */}
        <Pressable
          onPress={onResend}
          disabled={cooldown > 0 || resending}
          className="mt-1 flex-row items-center gap-1.5"
        >
          <MaterialIcons name="refresh" size={16} color={cooldown > 0 ? '#9ca3af' : passengerPurple[600]} />
          <Text
            className={`font-plex-bold text-sm ${
              cooldown > 0 ? 'text-neutral-400 dark:text-neutral-500' : 'text-brand-700 dark:text-brand-300'
            }`}
          >
            {resending
              ? '…'
        ...
        <Link
          href="/(auth)/sign-in"
          className="mt-1 font-plex-medium text-sm text-neutral-500 dark:text-neutral-400"
        >
          {t('auth.backToSignIn')}
        </Link>
      </KeyboardAvoidingView>
