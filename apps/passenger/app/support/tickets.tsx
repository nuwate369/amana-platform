...
/** شاشة «تذاكري» — قائمة تذاكر الدعم + زر إنشاء تذكرة. */
          ...
          {tickets.length === 0 ? (
            <View className="mt-24 items-center gap-3">
              <View className="h-20 w-20 items-center justify-center rounded-full bg-brand-50 dark:bg-neutral-800">
                <MaterialIcons name="support-agent" size={44} color={passengerPurple[400]} />
              </View>
            ...
            <View className="gap-3">
              {tickets.map((tk) => {
                ...
                return (
                  <Pressable
                    key={tk.id}
                    onPress={() => router.push(`/support/${tk.id}` as Href)}
