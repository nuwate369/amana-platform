...
import { listMyRides, type RideHistoryItem } from '@/lib/account';

/**
 * شاشة «سجل الرحلات» — رحلات الراكبة الحقيقية من Supabase (لا بيانات وهمية).
 * تصفية حسب الحالة، وبطاقة لكل رحلة بلقطة اسم السائقة والمسار والأجرة والتاريخ.
 */
      ...
      <View className="mb-4 flex-row items-start justify-between">
        <View className="flex-row items-center gap-4">
          <View className="h-12 w-12 items-center justify-center rounded-full bg-neutral-200 dark:bg-neutral-700">
            <MaterialIcons name={cancelled ? 'no-accounts' : 'person'} size={26} color={cancelled ? '#9ca3af' : passengerPurple[400]} />
          </View>
          <View>
            <Text className="font-plex-semibold text-base text-neutral-900 dark:text-neutral-100">
              {ride.driverName ?? (cancelled ? 'رحلة ملغاة' : 'سائقة أمانة')}
            </Text>
            {cancelled ? (
              ...
              <Text className="mt-0.5 font-plex-medium text-xs text-neutral-400">
      ...
      >
        <View className="mb-6">
          ...
          <Text className="font-plex text-sm text-neutral-500 dark:text-neutral-400">
            استعرضي جميع رحلاتك السابقة وتفاصيلها
          </Text>
        </View>
        ...
        {loading ? (
          ...
          <View className="items-center gap-3 py-16">
            <MaterialIcons name="history" size={56} color={passengerPurple[300]} />
            ...
            <Pressable
              ...
              className="mt-1 rounded-full bg-brand-600 px-6 py-2.5 active:scale-[0.98]"
            >
              <Text className="font-plex-semibold text-sm text-white">اطلبي رحلتك الأولى</Text>
            </Pressable>
          </View>
        ) : (
