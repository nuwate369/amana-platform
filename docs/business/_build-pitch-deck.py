...
# ═══════════════════════ 3 · حجم السوق ═══════════════════════
s = slide_light("حجم السوق", "سوق ينمو ١٣٪ سنويًّا — ونصفه بلا خدمة",
                "قطاع طلب سيارات الأجرة في المملكة، وأرقام الطلب النسائي التي تثبت جاهزية السوق.")
for i, (v, l, c) in enumerate([("١٫٩ مليار $", "حجم السوق ٢٠٢٥", PURPLE),
                               ("٥٫٦ مليار $", "المتوقَّع ٢٠٣٤", PURPLE),
                               ("١٣٫٠٣٪", "نموّ سنوي مركّب", GOLD),
                               ("٥٤٫٢٦٪", "نموّ الرحلات سنويًّا", GOLD)]):
...
text(s, "الجمهور المستهدف", 1.2, 4.25, 11.25, 0.35, size=15, bold=True, color=PURPLE)
table(s, 1.2, 4.7, 11.25, ["الشريحة", "الحجم التقريبي", "دافع الاستخدام"],
      [["الموظفات", "١٫٥ مليون", "تنقّل يومي للعمل يتطلّب خصوصية وانتظامًا"],
       ["ربّات المنازل", "٤ ملايين", "المجمّعات والزيارات العائلية والمستشفيات"],
       ["الطالبات", "١٫٢ مليون", "الجامعات والكلّيات والمعاهد"],
       ["الزائرات والسائحات", "١٠ ملايين سنويًّا", "استكشاف معالم المدن بخدمة نسائية"]],
      col_w=[2.6, 2.35, 6.3], row_h=0.44, size=12)
footer(s, 3)
...
feats = [
    ...
    ("👥", "النقل التشاركي", "مشاركة التكلفة بين الراكبات — خفض يصل إلى ٦٠٪."),
...
for i, (ic, t, b) in enumerate(feats):
    ...
    y = 2.5 + row * 2.05
    card(s, x, y, 2.62, 1.9, t, b, PURPLE if row == 0 else GOLD, ic, title_size=13)
footer(s, 5)
...
table(s, 1.2, 2.5, 11.25,
      ...
      [["وصليني", "~١٥٪ نسائيًّا", "أوّل تطبيق نسائي، ٦٠ مدينة", "بلا ذكاء اصطناعي ولا استدامة", "متوسّط"],
       ["مريني", "~١٠٪ نسائيًّا", "تركيز على الخصوصية", "مرحلة تطوير، تركيز ضيّق", "منخفض"],
       ["أوبر «سائقات»", "~٥٪ من أوبر", "علامة عالمية وشبكة ضخمة", "خدمة عامّة لا منصّة نسائية", "مرتفع"],
       ["كريم · جيني وغيرهما", "~٧٠٪", "تغطية وأسعار تنافسية", "لا خدمة نسائية حصرية", "منخفض"]],
      col_w=[2.65, 1.75, 2.95, 2.95, 0.95], row_h=0.58, size=12)
rect(s, 1.2, 5.55, 11.25, 1.0, fill=GOLD_PALE, radius=0.08)
text(s, "استراتيجية التفوّق: مزايا حصرية يصعب تقليدها (مرافقة الأطفال، البصمة الكربونية، المجموعات المغلقة)\nمع توسّع سريع في المدن الرئيسية قبل أن يتحرّك اللاعبون الكبار.",
     1.5, 5.75, 10.65, 0.65, size=13, bold=True, color=GOLD, align=PP_ALIGN.CENTER)
footer(s, 6)
...
table(s, 1.2, 2.55, 11.25,
      ...
      [["المدن المغطّاة", "١ (بريدة)", "٣", "٥ – ٧"],
       ...
       ["الرحلات سنويًّا", "٢٠ – ٢٥ ألف", "٢٠٠ – ٢٥٠ ألف", "٧٠٠ – ٨٠٠ ألف"],
       ["متوسّط قيمة الرحلة", "٣٠ ريال", "٣٠ ريال", "٣٠ ريال"]],
...
s = slide_light("الأطروحة الاستثمارية", "التكلفة التقنية لكل رحلة تنخفض ٩٥٪",
                "فريق ثابت من ٨ أفراد وبنية سحابية مُدارة يخدمان ٢٠ ألف رحلة أو ٨٠٠ ألف رحلة بالهيكل نفسه.")
bars(s, 2.6, 2.75, 8.4, 2.45,
     [("السنة ١\n~٢٢ ألف رحلة", 40), ("السنة ٢\n~٢٢٥ ألف رحلة", 5), ("السنة ٣\n~٧٥٠ ألف رحلة", 2)],
     maxv=40, fmt=lambda v: f"{v} ر.س", highlight=2)
...
table(s, 1.2, 2.5, 11.25,
      ...
      [["البنية التحتية", "١٥٬٠٥٠", "٦٣٬٠٠٠", "١٧٨٬٠٠٠", "٢٥٦٬٠٥٠"],
       ["الذكاء الاصطناعي", "٢٬٢٥٠", "١١٦٬٠٠٠", "٢٤٧٬٠٠٠", "٣٦٥٬٢٥٠"],
       ["الفريق التقني", "١٬٠٦٦٬٠٠٠", "١٬١٤١٬٦٠٠", "١٬٢٢٠٬٥٠٠", "٣٬٤٢٨٬١٠٠"],
       ["الأمن والأدوات", "١٠٠٬٠٠٠", "١١٥٬٠٠٠", "١٧٥٬٠٠٠", "٣٩٠٬٠٠٠"],
       ["احتياطي طوارئ ١٠٪", "١١٨٬٣٣٠", "١٤٣٬٥٦٠", "١٨٢٬٠٥٠", "٤٤٣٬٩٤٠"]],
      col_w=[3.05, 2.05, 2.05, 2.05, 2.05], row_h=0.5, size=12.5,
      foot=["الإجمالي", "١٬٣٠١٬٦٣٠", "١٬٥٧٩٬١٦٠", "٢٬٠٠٢٬٥٥٠", "٤٬٨٨٣٬٣٤٠"])
...
table(s, 1.2, 2.55, 11.25,
      ["الدور", "العدد", "السنة ١", "السنة ٢", "السنة ٣"],
      [["قائد تقني (CTO)", "١", "٢٥٩٬٢٠٠", "٢٧٧٬٣٠٠", "٢٩٦٬٤٠٠"],
       ["مطوّر Full-stack", "١", "١٨٧٬٢٠٠", "٢٠٠٬٤٠٠", "٢١٤٬٢٠٠"],
       ["مطوّر مساعد", "٢", "٢٣٠٬٤٠٠", "٢٤٦٬٦٠٠", "٢٦٣٬٥٠٠"],
       ["مطوّر واجهات UI/UX", "١", "١٣٠٬٠٠٠", "١٤٠٬٠٠٠", "١٥٠٬٠٠٠"],
       ["دعم فنّي", "٣", "٢٥٩٬٢٠٠", "٢٧٧٬٣٠٠", "٢٩٦٬٤٠٠"]],
      col_w=[3.75, 1.4, 2.03, 2.03, 2.04], row_h=0.5, size=12.5,
      foot=["الإجمالي", "٨", "١٬٠٦٦٬٠٠٠", "١٬١٤١٬٦٠٠", "١٬٢٢٠٬٥٠٠"])
...
s = slide_light("النموذج المالي", "ربحية من السنة الأولى — بتقديرات محافظة",
                "خُفِّضت توقّعات الإيراد ٥٨٪ عن النسخة السابقة لتكون قابلة للدفاع أمام أي مراجعة.")
table(s, 1.2, 2.5, 7.35,
      ["مصدر الإيراد — السنة الأولى", "الأساس", "الإيراد"],
      [["عمولة الرحلات (٢٠٪)", "~٢٢ ألف رحلة × ٣٠ ريال", "٤٨٥٬٠٠٠"],
       ["اشتراكات «باقة التنقّل»", "٨٠٠ مشتركة × ١٤٩ ريال", "١٬٤٣٠٬٤٠٠"],
       ["الخدمات الإضافية والإعلانات", "~٣٠ ألف شهريًّا (آخر ٦ أشهر)", "١٨٠٬٠٠٠"]],
      col_w=[3.0, 2.75, 1.6], row_h=0.55, size=12,
      foot=["إجمالي الإيرادات", "", "~٢٬١٠٠٬٠٠٠"])
card(s, 8.85, 2.5, 3.6, 1.7, "التكاليف التشغيلية",
     "١٬٥٠٠٬٠٠٠ ريال سنويًّا — تشمل الميزانية التقنية الكاملة والتشغيل والتسويق.", PURPLE, "📉")
card(s, 8.85, 4.4, 3.6, 1.7, "صافي الربح — السنة ١",
     "~٦٠٠٬٠٠٠ ريال، تُعاد إلى التوسّع والتشغيل بدل توزيعها.", GREEN, "📈")
for i, (v, l) in enumerate([("~٧٢٥ ألف", "رأس المال المطلوب"),
                            ("١٢–١٨ شهرًا", "فترة الاسترداد"),
                            ("٣٠–٣٥٪", "العائد في السنة ٣")]):
    x = 1.2 + i * 2.5
    rect(s, x, 4.75, 2.35, 1.15, fill=PURPLE_PALE, radius=0.08)
    text(s, v, x, 5.0, 2.35, 0.4, size=17, bold=True, color=PURPLE, align=PP_ALIGN.CENTER)
    text(s, l, x, 5.45, 2.35, 0.32, size=11.5, color=GREY, align=PP_ALIGN.CENTER)
footer(s, 11)

# ═══════════ 12 · اقتصاديات الرحلة الواحدة ═══════════
s = slide_dark("اقتصاديات الوحدة", "الرحلة تصبح مربحة\nمنذ السنة الثانية")
for i, (v, l, c) in enumerate([("٦٫٠٠", "عمولة المنصّة من الرحلة", GOLD_PALE),
                               ("٢٫٠٠", "التكلفة التقنية · السنة ٣", RGBColor(0xC9, 0xB6, 0xDE)),
                               ("٤٫٠٠", "الفائض لكل رحلة", WHITE)]):
...
text(s, "التكلفة التقنية تهبط من ٤٠ ريالًا إلى ٢ — فكل رحلة إضافية بعد السنة الأولى تضيف هامشًا لا تكلفة.",
     0.9, 6.15, W - 1.8, 0.4, size=13, color=GOLD_PALE, align=PP_ALIGN.CENTER)
footer(s, 12)

# ═══════════════════ 13 · خارطة الطريق ═══════════════════
s = slide_light("التنفيذ", "خارطة الطريق — الإطلاق خلال ٨ أشهر",
                "رأس مال تأسيسي ~٧٢٥ ألف ريال موزّع على ستّ مراحل تنفيذية واضحة.")
phases = [
    ("١–٤", "التأسيس والترخيص الريادي", "خطاب تزكية الجامعة، خطاب عدم الممانعة، رخصة «ريادي»، السجل التجاري", "٥٠٬٠٠٠"),
    ("٢–٥", "التطوير التقني (MVP)", "تطبيقات Android و iOS، الربط مع منصّة هيئة النقل، واجهات احترافية", "١٥٠٬٠٠٠"),
    ("٤–٥", "التوظيف والتدريب", "استقطاب أوّل ١٠٠ سائقة وتأهيلهنّ على القيادة الآمنة وخدمة العملاء", "١٠٠٬٠٠٠"),
    ("٦", "الإطلاق التجريبي", "التشغيل في بريدة مع ٥٠ سائقة وجمع الملاحظات الميدانية", "٢٥٬٠٠٠"),
    ("٧–٨", "الإطلاق الرسمي", "حملة تسويق رقمية موجَّهة وإطلاق «باقة التنقّل»", "١٠٠٬٠٠٠"),
    ("١٢", "التوسّع الإقليمي", "الرياض وجدة والدمام — بتمويل من الإيرادات المتحقّقة", "٣٠٠٬٠٠٠"),
]
yy = 2.5
...
text(s, "~٧٢٥٬٠٠٠ ريال", 1.3, yy + 0.29, 3.0, 0.3, size=15, bold=True, color=GOLD_PALE, align=PP_ALIGN.LEFT)
footer(s, 13)

# ════════════════ 14 · رأس المال والتمويل ════════════════
s = slide_light("الطلب", "~٧٢٥ ألف ريال — رأس المال حتى الإطلاق",
                "مبلغ محسوب لا مبالغ فيه: ثلثه ضمان مسترَدّ لهيئة النقل، والباقي منتج وتسويق وتأسيس.")
table(s, 1.2, 2.6, 7.4, ["أوجه الاستخدام", "المبلغ", "النسبة"],
      [["الضمان المالي لهيئة النقل (مسترَدّ)", "٢٥٠٬٠٠٠", "٣٤٪"],
       ["تطوير التطبيق (MVP متكامل)", "١٥٠٬٠٠٠", "٢١٪"],
       ["التسويق والإطلاق (٦ أشهر)", "١٥٠٬٠٠٠", "٢١٪"],
       ["رواتب أوّل ٣ أشهر", "١٢٥٬٠٠٠", "١٧٪"],
       ["التأسيس والتراخيص القانونية", "٥٠٬٠٠٠", "٧٪"]],
      col_w=[4.0, 2.0, 1.4], row_h=0.55, size=12.5,
      foot=["الإجمالي", "٧٢٥٬٠٠٠", "١٠٠٪"])
card(s, 8.85, 2.6, 3.6, 1.75, "مصادر الدعم المستهدفة",
     "كفالة (ضمانات حتى ١٥ مليون) · رواد التقنية (٢٠٠–٣٠٠ ألف) · بنك التنمية الاجتماعية · حاضنة ابتكار بجامعة القصيم", GOLD, "🤝")
card(s, 8.85, 4.55, 3.6, 1.75, "العائد المتوقّع",
     "استرداد رأس المال خلال ١٢–١٨ شهرًا، وعائد ٣٠–٣٥٪ بحلول السنة الثالثة.", GREEN, "📈")
footer(s, 14)

# ════════════════ 15 · المسار النظامي ════════════════
s = slide_light("المسار النظامي", "الترخيص الريادي — مسار رسمي مكتمل",
                "رخصة تصدرها وزارة الاستثمار لروّاد الأعمال غير السعوديين: ملكية ١٠٠٪ وإقامة مستقلّة.")
steps = [("١", "خطاب التزكية", "من مركز الابتكار بجامعة القصيم — الشرط الأهمّ لقبول الطلب", "٢–٤ أسابيع"),
         ("٢", "خطاب عدم الممانعة", "من الكفيل الحالي لتحويل الإقامة", "١–٢ أسبوع"),
         ("٣", "رخصة «ريادي»", "التقديم عبر بوابة وزارة الاستثمار (MISA)", "١–٢ أسبوع"),
         ("٤", "تحويل الإقامة", "الإقامة الريادية عبر الجوازات — تشمل العائلة", "٢–٤ أسابيع"),
         ("٥", "تأسيس الشركة", "السجل التجاري، العلامة التجارية، ترخيص هيئة النقل", "١–٢ أسبوع")]
yy = 2.5
for i, (n, t, d, dur) in enumerate(steps):
    ...
    text(s, dur, 1.3, yy + 0.14, 1.6, 0.3, size=12, bold=True, color=GOLD, align=PP_ALIGN.LEFT)
    yy += 0.66
for i, (t, b) in enumerate([("ملكية ١٠٠٪", "بلا شريك محلّي"),
                            ("٢٬٠٠٠ ريال/سنة", "رسوم أوّل ٣ سنوات"),
                            ("إعفاء من نطاقات", "٣ سنوات تشغيلية"),
                            ("إقامة عائلية", "الزوج/ة والأبناء والوالدان")]):
    x = W - 1.2 - (i + 1) * 2.83 + 0.15
    rect(s, x, 6.0, 2.62, 0.85, fill=GOLD_PALE, radius=0.1)
    text(s, t, x, 6.13, 2.62, 0.3, size=13, bold=True, color=GOLD, align=PP_ALIGN.CENTER)
    text(s, b, x, 6.47, 2.62, 0.28, size=10.5, color=GREY, align=PP_ALIGN.CENTER)
footer(s, 15)

# ═════════════ 16 · التوافق مع رؤية ٢٠٣٠ ═════════════
...
table(s, 1.2, 2.5, 11.25, ["المخاطرة", "استراتيجية التخفيف", "خطّة الطوارئ"],
      [["قانونية — رفض التزكية أو الترخيص",
        ...
        "حاضنات بديلة (واعد، مسك)، أو شريك سعودي، أو تأشيرة استثمارية"],
       ["مالية — تجاوز التكاليف أو نقص الإيراد",
        "احتياطي طوارئ ١٠٪، وتوقّعات مخفَّضة ٥٨٪ أصلًا",
        "خفض المصاريف التشغيلية والتركيز على النموّ العضوي"],
      ...
      col_w=[3.1, 4.05, 4.1], row_h=0.72, size=11.5)
footer(s, 17)
...
whys = [("السوق مفتوح", "لا منافس نسائي متكامل في المملكة حتى اليوم"),
        ...
        ("الاقتصاد سليم", "ربحية من السنة الأولى واسترداد خلال ١٢–١٨ شهرًا"),
